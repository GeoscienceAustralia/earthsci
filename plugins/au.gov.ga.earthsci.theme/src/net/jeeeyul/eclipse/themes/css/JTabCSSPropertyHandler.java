...
/**
 * A CSS property handlers for {@link CTabFolder} that uses
 * {@link JeeeyulsTabRenderer} as renderer.
 * 
...
public class JTabCSSPropertyHandler implements ICSSPropertyHandler
{
	...
	public boolean applyCSSProperty(final Object element, final String property, final CSSValue value,
			final String pseudo, @Extension final CSSEngine engine) throws Exception
	{
				...
				else
				{
					...
					if ((value instanceof CSSPrimitiveValue))
					{
						boolean _xblockexpression_1 = false;
						{
							...
							tabFolder.setSelectionBackground(new Color[] { newColor, newColor }, new int[] { 100 },
									true);
	...
	{
		@Extension
		final JTabRendererHelper JTabRendererHelper =
				new net.jeeeyul.eclipse.themes.rendering.internal.JTabRendererHelper();
		CTabFolderElement tabFolderElement = ((CTabFolderElement) element);
		Object _nativeWidget = tabFolderElement.getNativeWidget();
		CTabFolder tabFolder = ((CTabFolder) _nativeWidget);
		CTabFolderRenderer _renderer = tabFolder.getRenderer();
	...
	private String toHTML(final HSB[] colors, final int[] percents)
	{
		...
		else
		{
			...
			String _join =
					IterableExtensions.<HSB> join(((Iterable<HSB>) Conversions.doWrapArray(colors)), " ", _function);
			...
			String _join_1 = IterableExtensions.<Integer> join(((Iterable<Integer>) Conversions.doWrapArray(percents)),
					" ", _function_1);
	...
	private String toHTML(final Color[] colors, final int[] percents)
	{
		...
		else
		{
			...
			String _join = IterableExtensions.<Color> join(((Iterable<Color>) Conversions.doWrapArray(colors)), " ",
					_function);
			_builder.append(_join, "");
			_builder.append(" ");
			final Function1<Integer, String> _function_1 = new Function1<Integer, String>()

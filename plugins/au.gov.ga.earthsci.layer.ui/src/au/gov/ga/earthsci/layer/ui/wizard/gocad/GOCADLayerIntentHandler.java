...
public class GOCADLayerIntentHandler extends AbstractRetrieveIntentHandler
{
	...
	protected void handle(final IRetrievalData data, final URL url, final Intent intent, final IIntentCallback callback)
	{
		try
		{
			final String[] properties = readProperties(data);
			shell.getDisplay().asyncExec(new Runnable()
			{
				...
				public void run()
				{
					try
					{
						...
						boolean isSGrid = extension.toLowerCase().equals("sg"); //$NON-NLS-1$

						final GOCADLayerParameters params = new GOCADLayerParameters();
						WizardDialog dialog =
								new WizardDialog(shell, new GOCADLayerParametersWizard(params, properties));
						...
						String layerType = isSGrid ? "VolumeLayer" : "ModelLayer"; //$NON-NLS-1$ //$NON-NLS-2$
						String dataFormat = isSGrid ? "GOCAD SGrid" : "GOCAD"; //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected String[] readProperties(IRetrievalData data) throws IOException
	{

...
public class CameraPropertyPersisterTest
{
			...
			{
				{
					...
					will(returnValue("not.a.camera.property"));
				}
			}
		});
	...
	private CameraProperty createCameraProperty(double eyeLat, double eyeLon, double eyeEl, double lookatLat,
			double lookatLon, double lookatEl, double upw, double upx, double upy, double upz)
	{
		CameraProperty p =
				new CameraProperty(Position.fromDegrees(eyeLat, eyeLon, eyeEl), Position.fromDegrees(lookatLat,
						lookatLon, lookatEl), new Vec4(upx, upy, upz, upw));
		return p;
	}
}

...
/**
 * An {@link IBookmarkPropertyAnimator} that can animate the world layer state between
 * the two given property states.
 * <p/>
 * Animation semantics are as follows:
 * <ol>
 * 	<li> Layer matching is done solely on layer URI
 * 	<li> Layers that appear in the current world model but neither the start nor end property
 * 	     are animated between their current opacity value and 0.0 (off)
 *  <li> Layers that appear in the start and/or end properties but NOT in the current world
 *  	 model are ignored
 * 	<li> Layers that appear in both start and end properties AND in the current world model
 * 		 are animated between the opacities stored in the start and end properties
 *  <li> Layers that appear in the start property but NOT the end property are animated between 
 *  	 the start property opacity and 0.0 (off)
 *  <li> Layers that appear in the end property but NOT the start property are animated between
 *  	 their current opacity and the end property opacity
 *  <li> A layer which is disabled in the current world model will be assigned an opacity value
 *       of 0.0 where the world value is used above
 * </ol>
 * 
 * The above rules are enumerated in the matrix below (W = world state, S = Start property state,
 * E = End property state, O = Off, I = Ignore).
 * 
 * <pre>
 * W    S    E   |  Animate
 * --------------------------
...
public class LayersPropertyAnimator extends AbstractBookmarkPropertyAnimator
{
	...
	public LayersPropertyAnimator(final ITreeModel model, 
								  final LayersProperty start, 
								  final LayersProperty end, 
								  long duration)
	{
	...
	public void init()
	{
		...
		for (Layer l : model.getLayers())
		{
			...
			animationVectors.put(layerNode, new Vector2(getStartOpacity(layerNode), 
														getEndOpacity(layerNode)));
		}
		initialised = true;
	}

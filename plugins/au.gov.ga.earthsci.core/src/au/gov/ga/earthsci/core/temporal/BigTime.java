 ...
 * as an arbitrary precision integer as the number of nanoseconds since 
 * {@code 0:00:00:0000 January 1, 1970 UTC} (i.e. the standard Unix date epoch).
 * <p/>
 * All times are represented in UTC.
 * <p/>
 * While time instants can be represented to nanosecond precision, each {@link BigTime} instance includes
 * a {@code resolution} in nanoseconds, which gives an indication of the scale at which the time instant can be 
 * considered useful. For example, a time instant obtained from a Java {@link Date} object might
 * have a resolution of 1 second, while an instance representing a geological time period may have
 * a resolution of 1 million years. It would be programmer error to use a {@link BigTime} instance
 * at a resolution higher than it's inherent resolution.
 * <p/>
 * This class can be used in place of the standard {@link Date} class when either:
 * <ul>
 * 	<li>Precision higher than millisecond is required; or
 * 	<li>Dates greater than ~292 Million Years (past or future) need to be represented (e.g. geological time scales)
 * </ul>
 * <p/>
 * <b>Note:</b> The natural ordering of this class (implemented in {@link Comparable}) is inconsistent
 * with {@code equals}. Two instances are considered equal via the {@link #equals(Object)} method if
 * they have the same time instant AND the same resolution. The {@link #compareTo(BigTime)} method,
 * on the other hand, compares two instances <em>at the lowest common resolution</em>.
 * <br/>
 * For example, a time of {@code [2010-01-01 12:00:00:000 resolution DAY]} is not equal to 
 * {@code [2010-01-01 12:05:01:000 resolution SECOND]} via {@link #equals(Object)}, but is via 
 * {@link #compareTo(BigTime)}.  
 * 
...
public class BigTime implements Comparable<BigTime>
{
	...
	public static final BigInteger NANOS_IN_YEAR = BigInteger.valueOf((long)(NANOS_IN_DAY.doubleValue() * DAYS_IN_YEAR));
	...
	public static final BigInteger EARLIEST_DATE_VALUE_IN_NANOS = BigInteger.valueOf(Long.MIN_VALUE).multiply(NANOS_IN_MILLISECOND);
	public static final BigInteger LARGEST_DATE_VALUE_IN_NANOS = BigInteger.valueOf(Long.MAX_VALUE).multiply(NANOS_IN_MILLISECOND);
	
	/**
	 * Create a new {@link BigTime} instance that represents the same time as the given {@link Date}.
	 * 
	 * @return A new {@link BigTime} instance that represents the same time as the given {@link Date},
	 ...
	 * 
	 * @return a new {@link BigTime} instance representing the current instant in time (to millisecond resolution)
	 */
	...
	/** 
	 * The number of nanoseconds since epoch ({@code 0:00:00:0000 January 1, 1970 UTC}) 
	 * this instance represents.
	 */
	private BigInteger nansecondsSinceEpoch;
	
	/**
	 * The resolution of this instance, in nanoseconds. This is an indication of the minimum scale at which this 
	 * instance can be considered useful.
	 * <p/>
	...
	/**
	 * Create a new instance with the given nanosecond offset and nanosecond resolution
	 */
	...
	public boolean equals(Object obj)
	{
		...
		return other.nansecondsSinceEpoch.equals(this.nansecondsSinceEpoch) &&
				other.resolution.equals(this.resolution);
	}
	...
	/**
	 * @return the time this instance represents, in nanoseconds since epoch. Should always 
	 * be used in conjunction with the {@link #resolution}.
	 * 
	 ...
	 * 
	 * @return <code>true</code> if this instance is within the range of the {@link Date} class
	 */
	...
	/**
	 * Returns the {@link Date} representation of the time this instance represents, if it is possible.
	 *  
	 * @return The {@link Date} representation of this instance; or <code>null</code> if this instance
	 * cannot be represented as a {@link Date}.
	 * 
	 * @see #isInDateRange()
	 */
	public Date getDate()
	...
	/**
	 * Return a new {@link BigTime} instance that represents the same time instant at the resolution
	 * of this instance, but with all information at higher resolutions removed.
	 * 
	 * @return A new instance that represents the same time instant, but with all higher resolution
	 * information removed.
	 */
	...
	/**
	 * Return a new {@link BigTime} instance that represents the same time instant at the provided
	 * resolution.
	 * <p/>
	 ...
	 * 
	 * @param resolution The resolution to change to
	 * 
	 * @return A new instance representing the same time at the provided resolution
	 */
	...
	/**
	 * Calculate a normalised version of the provided nanosecond value to remove any information
	 * at resolutions higher than the provided value.
	 * <p/>
	 * For example, a value of 1021 normalised to microsecond resolution will return 1000.
	 * 
